(function() {
    ...
    function displayUserDetails(userDetails) {
        ...
        const fields = [
            { label: 'Username', value: userDetails.name },
            { label: 'Profile ID', value: userDetails.id },
            { label: 'Culture', value: userDetails.culture },
            { label: 'Created', value: new Date(userDetails.created).toLocaleString() },
            { label: 'Latest Login (j68d)', value: userDetails.latestLogins.find(login => login.game === 'j68d')?.timestamp || 'N/A' }
        ];
